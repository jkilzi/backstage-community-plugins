import { LongTermRecommendationBoxPlots, MediumTermRecommendationBoxPlots, PlotsData, RecommendationBoxPlotsRecommendations, ShortTermRecommendationBoxPlots } from "@backstage-community/plugin-resource-optimization-common";
import { Interval, UsageType } from "../types/chart";
import { format } from 'date-fns';


export const getRecommendationTerm = (recommendations: RecommendationBoxPlotsRecommendations, interval: Interval)
              : ShortTermRecommendationBoxPlots | MediumTermRecommendationBoxPlots | LongTermRecommendationBoxPlots => {
  let result;
  switch (interval) {
...
export const createUsageDatum = (usageType: UsageType, currentInterval: Interval, recommendations: RecommendationBoxPlotsRecommendations) => {
    ...
    for (const key of Object.keys(plotsData)) {
      ...
      const xVal = currentInterval === Interval.shortTerm ? format(date, 'kk:mm') : format(date, 'MMM d');
      datum.push({
        key,
        name: usageType,
        units: data?.format,
    ...
    if (datum.length === 0 && recommendations?.monitoringEndTime) {
      if (currentInterval === Interval.shortTerm) {
        ...
        for (let day = currentInterval === Interval.longTerm ? 15 : 7; day > 0; day--) {
          const today = new Date(recommendations?.monitoringEndTime);
