export const getTimeFromNow = (lastDate: string = 'N/A') => {
    ...
    if (Math.abs(diff) > WEEK_IN_MILLIS) {
        ...
        return intl.format(Math.trunc(-(diff % DAY_IN_MILLIS) / HOUR_IN_MILLIS), 'hour');
    } else if (Math.abs(diff) > MIN_IN_MILLIS) {
        return intl.format(Math.trunc(-(diff % HOUR_IN_MILLIS) / MIN_IN_MILLIS), 'minute');
    } else {
        return intl.format(Math.trunc(-(diff % MIN_IN_MILLIS) / SEC_IN_MILLIS), 'second');
    }
  };
