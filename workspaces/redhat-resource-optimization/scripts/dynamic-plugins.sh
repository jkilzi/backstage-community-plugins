...
Sub-commands:
  ...
  oci      Generates an OCI artifact containing the dynamic plugins.
EOF
}
...
function tgz {
  ...
  tar -vczf $artifact_name.tgz $artifact_name
  tar -vczf $artifact_name-backend.tgz $artifact_name-backend
...
EOF
  ...
  for dir in $(find . -type d -maxdepth 1 | tail -n +2); do
    rm -rf "$dir"
  done
  cd "$workspace_dir"
}
